...
class PreProcessing:
    ...
    def __init__(self,data_src):
        ...
        print("Loading VEIN Dataset...")
        self.images_train, self.images_test, self.labels_train, self.labels_test = self.preprocessing(1)#0.8
        ...
        for directory in os.listdir(self.data_src):
            try:
                for pic in os.listdir(os.path.join(self.data_src, directory)):
                    g_img = imread(os.path.join(self.data_src, directory, pic))
                    img = cv2.cvtColor(g_img,cv2.COLOR_GRAY2RGB)
